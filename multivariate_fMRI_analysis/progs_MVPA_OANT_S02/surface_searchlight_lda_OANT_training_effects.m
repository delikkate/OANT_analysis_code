...
% data from S02 investigating the effects of training (across word classes).

% targets:
% {S2_NU, S2_VU} = 1;
% {S2_NT, S2_VT} = 2.

% created by KD 18-12-2017

function surface_searchlight_lda_OANT_training_effects(subNum, hemisphere)
...
lda_results.sa.labels={sprintf('Subject SUB%02d: untrained vs. trained', subNum)};

%% save the resulting accuracy map as an .smp
data_output_fn = sprintf('SUB%02d_training_effects_accuracy_map_%s.smp', subNum, hemisphere);
