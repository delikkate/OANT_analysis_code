...
% .mat files with the updated codes into "log_S01".


function recode_conditions_OANT_S01(subNum, runNum, session)

% UPD by KD: 25-01-2018
% example call: recode_conditions_OANT_S01(1, 1, 'pre')
...
for i = 1:length(ID)
    if (mod(PicNum(i),2) == 0)
        ID(i) = 5; % assign a new condition code to squiggles
    end
end
...
newPathToLog = [pwd sprintf('\\log_S01\\SUB%02d_RUN%02d_S%02d_OANT.mat', subNum, runNum, sesNum)];
